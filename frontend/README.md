...
- 调用 `/api/papers/analyze` 生成分析结果；后端配置有效 LLM Key 时，结果来自真实模型分析，否则使用占位分析。
...
当前问答答案仍由后端关键词检索和模板生成，引用片段来自 `chunks.json`。下一阶段后端会升级为“先检索 citations，再让 LLM 基于引用片段生成自然语言回答”，前端届时主要需要增强引用片段展示和加载状态。
